...
def test_basic(fixed_translator: FixedTranslator, basic_input: bytes, basic_output: bytes) -> None:
    assert fixed_translator.translate(
        basic_input) == [basic_output]


def test_ignore_first_bytes(fixed_translator: FixedTranslator, input_with_prefix: bytes, basic_output: bytes) -> None:
    assert fixed_translator.translate(input_with_prefix) == [basic_output]


def test_2_inputs_1_output(fixed_translator: FixedTranslator, inputs_2: List[bytes], basic_output: bytes) -> None:
    assert fixed_translator.translate(inputs_2[0]) == [basic_output]
    assert fixed_translator.translate(inputs_2[1]) == [basic_output]


def test_3_inputs_2_outputs(fixed_translator: FixedTranslator, inputs_3: List[bytes], basic_output: bytes) -> None:
...
def test_4_inputs_3_outputs(fixed_translator: FixedTranslator, inputs_4: List[bytes], basic_output: bytes) -> None:
